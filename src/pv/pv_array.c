...
static pvp_array_data *pvp_array_get_data(pv val) {
	pvp_array_data *a = (pvp_array_data*)val.data;
	return a;
}

static uint32_t pvp_array_length(pvp_array_data *a) {
	return a->length;
}
...
static int pv_array_equal_self(pv val1, pv val2) {
	pvp_array_data *a1 = pvp_array_get_data(val1);
	pvp_array_data *a2 = pvp_array_get_data(val2);
...
static pvp_array_data *pvp_array_alloc(size_t size) {
	pvp_array_data *a = pv_alloc(sizeof(pvp_array_data) + sizeof(pv) * size);
	a->refcnt = PV_REFCNT_INIT;
  a->alloc_length = (uint32_t)size;
  return a;
}

static pvp_array_data *pvp_array_realloc(pvp_array_data *ain, size_t size) {
	...
	if (pvp_refcnt_unshared(&(ain->refcnt))) {
		...
		// 	return ain;
		// }
		a = pv_realloc(ain, sizeof(pvp_array_data) + sizeof(pv) * size);
	} else {
		// there is more then one copy of this pv, so i have to 
		a = pv_alloc(sizeof(pvp_array_data) + sizeof(pv) * size);
		memcpy(a, ain, sizeof(pvp_array_data) + sizeof(pv) * ain->length);
...
pv pv_array_concat(pv val1, pv val2) {
	...
	uint32_t l1 = pvp_array_length(a1);
	uint32_t l2 = pvp_array_length(a2);
	pvp_array_data *a = pvp_array_realloc(a1, max(a1->alloc_length, l1 + l2));
	a->length = l1 + l2;
