...
static pv parse_slice(pv j, pv slice, int* pstart, int* pend) {
  // Array slices
  pv start_pv = pv_object_get(pv_copy(slice), pv_string("start"));
...
pv pv_get(pv t, pv k) {
  ...
  if (pv_get_kind(t) == JV_KIND_OBJECT && pv_get_kind(k) == JV_KIND_STRING) {
    ...
    int start, end;
    pv e = parse_slice(pv_copy(t), k, &start, &end);
     ...
     */
    if (pv_get_kind(k) == JV_KIND_STRING && pv_string_length_bytes(pv_copy(k)) < 30) {
  ...
  if (pv_get_kind(k) == JV_KIND_STRING &&
    ...
    if (pv_get_kind(e) == JV_KIND_TRUE) {
      if (pv_get_kind(v) == JV_KIND_ARRAY) {
        int array_len = pv_array_length(pv_copy(t));
        assert(0 <= start && start <= end && end <= array_len);
        int slice_len = end - start;
        int insert_len = pv_array_length(pv_copy(v));
        if (slice_len < insert_len) {
          // array is growing
          int shift = insert_len - slice_len;
          for (int i = array_len - 1; i >= end; i--) {
            t = pv_array_set(t, i + shift, pv_array_get(pv_copy(t), i));
          }
        ...
        for (int i=0; i < insert_len; i++) {
          t = pv_array_set(t, start + i, pv_array_get(pv_copy(v), i));
        }
        pv_free(v);
      } else {
        pv_free(t);
...
static pv pv_dels(pv t, pv keys) {
  ...
  if (pv_get_kind(t) == JV_KIND_NULL || pv_array_length(pv_copy(keys)) == 0) {
    // no change
  } else if (pv_get_kind(t) == JV_KIND_ARRAY) {
    ...
    pv_array_foreach(keys, i, key) {
      if (pv_get_kind(key) == JV_KIND_NUMBER) {
        ...
        int start, end;
        pv e = parse_slice(pv_copy(t), key, &start, &end);
        if (pv_get_kind(e) == JV_KIND_TRUE) {
    ...
    int neg_idx = 0;
    int nonneg_idx = 0;
    int len = pv_array_length(pv_copy(t));
    pv_array_foreach(t, i, elem) {
      int del = 0;
      while (neg_idx < pv_array_length(pv_copy(neg_keys))) {
        int delidx = len + (int)pv_number_get_value_and_consume(pv_array_get(pv_copy(neg_keys), neg_idx));
        if (i == delidx) {
          del = 1;
        }
        if (i < delidx) {
      ...
      while (nonneg_idx < pv_array_length(pv_copy(nonneg_keys))) {
        int delidx = (int)pv_number_get_value_and_consume(pv_array_get(pv_copy(nonneg_keys), nonneg_idx));
        if (i == delidx) {
          del = 1;
        }
        if (i < delidx) {
      ...
      for (int sidx=0; !del && sidx<pv_array_length(pv_copy(starts)); sidx++) {
        if ((int)pv_number_get_value_and_consume(pv_array_get(pv_copy(starts), sidx)) <= i &&
            i < (int)pv_number_get_value_and_consume(pv_array_get(pv_copy(ends), sidx))) {
          del = 1;
        }
      }
      if (!del)
...
pv pv_setpath(pv root, pv path, pv value) {
  ...
  if (pv_array_length(pv_copy(path)) == 0) {
    pv_free(path);
    pv_free(root);
    return value;
  }
  pv pathcurr = pv_array_get(pv_copy(path), 0);
  pv pathrest = pv_array_slice(path, 1, pv_array_length(pv_copy(path)));
  ...
  if (pv_get_kind(pathcurr) == JV_KIND_OBJECT) {
    ...
    return pv_set(root, pathcurr,
                  pv_setpath(pv_get(pv_copy(root), pv_copy(pathcurr)), pathrest, value));
  }

  pv subroot = pv_get(pv_copy(root), pv_copy(pathcurr));
  ...
  // to null first.
  root = pv_set(root, pv_copy(pathcurr), pv_null());
  if (!pv_is_valid(root)) {
    pv_free(pathcurr);
    pv_free(pathrest);
    pv_free(value);
...
static pv delpaths_sorted(pv object, pv paths, int start) {
  pv delkeys = pv_array();
  for (int i=0; i<pv_array_length(pv_copy(paths));) {
    int j = i;
    assert(pv_array_length(pv_array_get(pv_copy(paths), i)) > start);
    int delkey = pv_array_length(pv_array_get(pv_copy(paths), i)) == start + 1;
    pv key = pv_array_get(pv_array_get(pv_copy(paths), i), start);
    while (j < pv_array_length(pv_copy(paths)) &&
           pv_equal(pv_copy(key), pv_array_get(pv_array_get(pv_copy(paths), j), start)))
      j++;
    ...
    if (delkey) {
      ...
      pv subobject = pv_get(pv_copy(object), pv_copy(key));
      if (!pv_is_valid(subobject)) {
        ...
        pv newsubobject = delpaths_sorted(subobject, pv_array_slice(pv_copy(paths), i, j), start+1);
...
pv pv_delpaths(pv object, pv paths) {
  ...
  paths = pv_sort(paths, pv_copy(paths));
  ...
  if (pv_array_length(pv_copy(paths)) == 0) {
    // nothing is being deleted
    pv_free(paths);
    return object;
  }
  if (pv_array_length(pv_array_get(pv_copy(paths), 0)) == 0) {
...
static int string_cmp(const void* pa, const void* pb){
  const pv* a = pa;
  const pv* b = pb;
  int lena = pv_string_length_bytes(pv_copy(*a));
  int lenb = pv_string_length_bytes(pv_copy(*b));
...
pv pv_keys_unsorted(pv x) {
  if (pv_get_kind(x) != JV_KIND_OBJECT)
    return pv_keys(x);
  pv answer = pv_array_sized(pv_object_length(pv_copy(x)));
...
pv pv_keys(pv x) {
  if (pv_get_kind(x) == JV_KIND_OBJECT) {
    int nkeys = pv_object_length(pv_copy(x));
...
int pv_cmp(pv a, pv b) {
  ...
  case JV_KIND_NUMBER: {
    if (pvp_number_is_nan(a)) {
      r = pv_cmp(pv_null(), pv_copy(b));
    } else if (pvp_number_is_nan(b)) {
      r = pv_cmp(pv_copy(a), pv_null());
    } else {
  ...
  case JV_KIND_ARRAY: {
    ...
    int i = 0;
    while (r == 0) {
      int a_done = i >= pv_array_length(pv_copy(a));
      int b_done = i >= pv_array_length(pv_copy(b));
      if (a_done || b_done) {
        r = b_done - a_done; //suddenly, logic
        break;
      }
      pv xa = pv_array_get(pv_copy(a), i);
      pv xb = pv_array_get(pv_copy(b), i);
      r = pv_cmp(xa, xb);
      i++;
    }
    break;
  ...
  case JV_KIND_OBJECT: {
    pv keys_a = pv_keys(pv_copy(a));
    pv keys_b = pv_keys(pv_copy(b));
    r = pv_cmp(pv_copy(keys_a), keys_b);
    if (r == 0) {
      pv_array_foreach(keys_a, i, key) {
        pv xa = pv_object_get(pv_copy(a), pv_copy(key));
        pv xb = pv_object_get(pv_copy(b), key);
        r = pv_cmp(xa, xb);
        if (r) break;
      }
    }
...
static int sort_cmp(const void* pa, const void* pb) {
  ...
  int r = pv_cmp(pv_copy(a->key), pv_copy(b->key));
...
static struct sort_entry* sort_items(pv objects, pv keys) {
  ...
  assert(pv_get_kind(keys) == JV_KIND_ARRAY);
  assert(pv_array_length(pv_copy(objects)) == pv_array_length(pv_copy(keys)));
  int n = pv_array_length(pv_copy(objects));
  struct sort_entry* entries = pv_mem_calloc(n, sizeof(struct sort_entry));
  for (int i=0; i<n; i++) {
    entries[i].object = pv_array_get(pv_copy(objects), i);
    entries[i].key = pv_array_get(pv_copy(keys), i);
    entries[i].index = i;
  }
  pv_free(objects);
  pv_free(keys);
...
pv pv_group(pv objects, pv keys) {
  ...
  if (n > 0) {
    ...
    for (int i = 1; i < n; i++) {
      if (pv_equal(pv_copy(curr_key), pv_copy(entries[i].key))) {
        pv_free(entries[i].key);
      } else {
        pv_free(curr_key);
